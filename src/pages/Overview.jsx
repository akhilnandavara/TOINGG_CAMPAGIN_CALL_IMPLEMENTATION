...
const Overview = () => {
  return (
    <>
      <h1 className="text-2xl sm:text-3xl font-bold text-DarkBg mb-4">Welcome to Toingg</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {contents.map((content, index) => (
          ...
          >
            <h2 className=" text-lg sm:text-2xl font-semibold mb-2">{content.title}</h2>
            <p className="text-DarkBg">{content.description}</p>
            <button className="mt-4 px-2 sm:px-4 py-2 bg-DarkBg text-white rounded-md">
              {content.button}
            </button>
          </div>
        ))}
